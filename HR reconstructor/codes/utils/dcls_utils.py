...
def normkernel_to_downkernel(rescaled_blur_hr, rescaled_hr, ksize, eps=1e-10):
    blur_img = torch.fft.fft2(rescaled_blur_hr, dim=(-3,-2,-1))
    blur_img = torch.stack((blur_img.real,blur_img.imag),-1)
    img = torch.fft.fft2(rescaled_hr, dim=(-3,-2,-1))
...
def convert_otf2psf(otf, size):
    ...
    ker[:, :, (centre-1):, (centre-1):] = (psf.real)[:, :, :centre, :centre]#.mean(dim=1, keepdim=True)
    ker[:, :, (centre-1):, :(centre-1)] = (psf.real)[:, :, :centre, -(centre-1):]#.mean(dim=1, keepdim=True)
    ker[:, :, :(centre-1), (centre-1):] = (psf.real)[:, :, -(centre-1):, :centre]#.mean(dim=1, keepdim=True)
    ker[:, :, :(centre-1), :(centre-1)] = (psf.real)[:, :, -(centre-1):, -(centre-1):]#.mean(dim=1, keepdim=True)
