...
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, img_hr, img_lr, img_lr_up, kernel, t=None, *args, **kwargs):
        x = img_hr
    ...
    def p_sample(self, x, t, cond, img_lr_up, noise_pred=None, clip_denoised=True, repeat_noise=False, kernel=False):
        if noise_pred is None:
            noise_pred = self.denoise_fn(x, t, cond=cond, img_lr_up=img_lr_up, kernel=kernel)
        b, *_, device = *x.shape, x.device
    ...
    @torch.no_grad()
    def sample(self, img_lr, img_lr_up, shape, kernel, save_intermediate=False):
        # print(shape)
